...
if not sessions:
await bot.send_message(admin_id, “❌ Нет активных сессий!”)
return
...
kb = InlineKeyboardMarkup(inline_keyboard=[
[InlineKeyboardButton(text=“❌ Удалить”, callback_data=f”delchat*{base64.b64encode(str(chat.chat_id).encode()).decode()}”)],
...
chats = await get_chats()
buttons = [[InlineKeyboardButton(text=f”{‘✅’ if c.chat_id in selected else ‘☑️’} {c.title}”, callback_data=f”selchat*{base64.b64encode(str(c.chat_id).encode()).decode()}”)] for c in chats]
buttons.append([InlineKeyboardButton(text=“✅ Готово”, callback_data=“done_selection”)])
...
```
    ...
    err = str(e).lower()
    if "locked" in err:
        await message.answer("❌ Database locked. Подождите 10 сек и попробуйте снова.")
    else:
        await message.answer(f"❌ Ошибка подключения: {str(e)[:300]}")
    if client:
        try:
...
```
    if ok:
        ...
        if join_results:
            await message.answer("📋 Результаты вступления:\n" + "\n".join(join_results))
    else:
...
sessions = await get_all_sessions()
if not sessions:
await call.message.answer(“❌ Нет сессий”)
await call.answer()
return
