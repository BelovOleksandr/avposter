...
print(”[ERR] API_ID и API_HASH не заданы”)
sys.exit(1)
...
async def add_session_safe(session_name: str, phone: str, first_name: str = None, username: str = None):
async def _do():
async with AsyncSessionLocal() as s:
ex = await s.execute(select(SessionAccount).where(SessionAccount.session_name == session_name))
if ex.scalar_one_or_none():
return False
s.add(SessionAccount(session_name=session_name, phone=phone, first_name=first_name, username=username))
await s.commit()
return True
...
async with AsyncSessionLocal() as s:
r = await s.execute(select(SessionAccount).where(SessionAccount.is_active == True, SessionAccount.is_working == True))
return r.scalars().all()
...
“”“Вступление в чат, возвращает True если успешно”””
# 1. По username
if chat_obj and chat_obj.username:
try:
await client.join_chat(f”@{chat_obj.username}”)
return True
...
if chat_obj and chat_obj.invite_link:
try:
await client.join_chat(chat_obj.invite_link)
return True
except UserAlreadyParticipant:
return True
except Exception:
...
“””
Антибан-рассылка:
- Каждый аккаунт шлёт во все нужные чаты
- Случайные задержки между сообщениями
- Автовступление в чаты перед рассылкой
- Контроль дневного лимита
- При FloodWait - пауза и продолжение
“””
sessions = await get_active_sessions()
if not sessions:
await bot.send_message(admin_id, “[ERR] Нет активных сессий!”)
return
...
status_msg = await bot.send_message(
    admin_id,
    f"[>>] Рассылка запущена\n[STAT] Аккаунтов: {len(sessions)}\n[MSG] Чатов: {len(chat_ids)}"
)
    ...
    if (idx + 1) % 3 == 0:
        try:
            await bot.edit_message_text(
                ...
                text=f"[>>] Рассылка...\n[OK] Успешно: {success_count}\n[ERR] Ошибок: {error_count}\n[SKIP] Пропущено: {skip_count}\n[MOB] Аккаунтов обработано: {idx+1}/{len(sessions)}"
            )
        except Exception:
            pass
...
report = (
    ...
    f"[OK] Успешно: <b>{success_count}</b>\n"
    f"[ERR] Ошибок: <b>{error_count}</b>\n"
    f"[SKIP] Пропущено (лимит): <b>{skip_count}</b>\n"
    f"[STAT] Всего чатов: {len(chat_ids)}\n"
    f"[MOB] Аккаунтов использовано: {len(sessions)}"
)
try:
...
sessions = await get_active_sessions()
if not sessions:
return [”[ERR] Нет активных сессий”]
...
for s in sessions:
    ...
    try:
        ...
        if ok:
            results.append(f"[OK] {s.phone} - вступил")
        else:
            results.append(f"[ERR] {s.phone} - не удалось вступить")
    except Exception as e:
        results.append(f"[ERR] {s.phone} - {str(e)[:60]}")
    finally:
        if client:
            try:
                await client.stop()
...
[InlineKeyboardButton(text=”[FIRE] О боте”, callback_data=“about”),
InlineKeyboardButton(text=”[GEM] Подписки”, callback_data=“subscriptions_menu”),
InlineKeyboardButton(text=”[$] Баланс”, callback_data=“my_balance”)],
[InlineKeyboardButton(text=”[TEL] Контакты”, callback_data=“contacts”)]
])

def kb_admin():
return InlineKeyboardMarkup(inline_keyboard=[
[InlineKeyboardButton(text=”[+] Добавить чат”, callback_data=“add_chat”),
InlineKeyboardButton(text=”[DIR] Список чатов”, callback_data=“list_chats”),
InlineKeyboardButton(text=”?? Рассылка”, callback_data=“create_broadcast”)],
[InlineKeyboardButton(text=”[STAT] Статистика”, callback_data=“show_stats”),
InlineKeyboardButton(text=”[SET]? Настройки”, callback_data=“settings”),
InlineKeyboardButton(text=”[USERS] Пользователи”, callback_data=“list_users”)],
[InlineKeyboardButton(text=”[CARD] Платежи”, callback_data=“payments_menu”),
InlineKeyboardButton(text=”[DATE] Подписки”, callback_data=“subscriptions_admin”),
InlineKeyboardButton(text=”[MOB] Сессии”, callback_data=“manage_sessions”)],
[InlineKeyboardButton(text=”[FIND] Чеккер акков”, callback_data=“check_accounts”),
InlineKeyboardButton(text=”[STAT] Статистика акков”, callback_data=“accounts_stats”),
InlineKeyboardButton(text=”[DOOR] Вступить в чат”, callback_data=“join_chat_manual”)],
[InlineKeyboardButton(text=”[HOME] Главное меню”, callback_data=“back_to_main”)]
])
...
text = (
“[FIRE] <b>REDLINE 911 TURBO</b> [FIRE]\n”
...
await message.answer_photo(FSInputFile(BANNER_FILE), caption=text, reply_markup=kb_main())
elif os.path.exists(WELCOME_VIDEO_FILE):
await message.answer_video(FSInputFile(WELCOME_VIDEO_FILE), caption=text, reply_markup=kb_main())
else:
...
return
await message.answer(”[SET]? <b>Админ-панель</b>”, reply_markup=kb_admin())
...
pass
await call.message.answer(”[SET]? <b>Админ-панель</b>”, reply_markup=kb_admin())
await call.answer()
...
text = (
f”[$] <b>Ваш баланс</b>\n\n”
f”[GEM] Баланс: <b>{(user.balance or 0.0):.2f}?</b>\n”
f”[DATE] Подписка: {sub_text}”
)
kb = InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text=”? Назад”, callback_data=“back_to_main”)]])
try:
await call.message.delete()
...
buttons += [
    [InlineKeyboardButton(text="[CARD] Карта", callback_data=f"pay_card_{plan}"),
...
await call.answer(”[ERR] Недостаточно средств”, show_alert=True)
return
now = datetime.now()
delta = {“3days”: timedelta(days=3), “week”: timedelta(weeks=1), “month”: timedelta(days=30), “year”: timedelta(days=365)}
...
f”[OK] <b>Подписка активирована!</b>\n\n[DATE] {plan} до {end.strftime(’%d.%m.%Y %H:%M’)}”,
reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text=”[HOME] Главное меню”, callback_data=“back_to_main”)]])
)
...
price = SUBSCRIPTION_PRICES.get(plan, 0)
text = f”[CARD] <b>Оплата картой</b>\n\n{price}? ? карта: <code>2202 2012 3456 7890</code>\n\nОтправьте чек администратору.”
kb = InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text=”? Назад”, callback_data=f”buy_sub*{plan}”)]])
...
[InlineKeyboardButton(text=”? Назад”, callback_data=f”buy_sub*{plan}”)]
])
await call.message.edit_text(text, reply_markup=kb)
await call.answer()
...
return
await call.message.answer(
“[NOTE] <b>Добавление чата</b>\n\n”
“Отправьте username (@channel), ID чата или перешлите сообщение из нужного чата.\n\n”
“[!]? Все активные сессии автоматически вступят в этот чат.”
)
...
if message.forward_origin and hasattr(message.forward_origin, 'chat'):
    ...
    if 't.me/+' in text or 'joinchat' in text:
        ...
        try:
            ...
            await message.answer(f"[ERR] Не удалось получить чат: {e}")
            return
    elif 't.me/' in text:
        ...
        try:
            ...
            await message.answer(f"[ERR] Ошибка: {e}")
            return
    else:
...
if not chat_id:
    await message.answer("[ERR] Не удалось определить чат.")
    return

existing = await get_chat_by_chat_id(chat_id)
if existing:
    await message.answer(f"[!]? Чат <b>{title}</b> уже добавлен.")
    await state.clear()
    return
...
await message.answer(f"[OK] Чат <b>{title}</b> добавлен!\n\n? Подключаю все сессии...")
...
pass
await call.message.answer(f”[DIR] <b>Чаты</b> (всего: {len(chats)}):”, reply_markup=kb)
await call.answer()
...
return
try:
chat_id = int(base64.b64decode(call.data.split(”*”, 1)[1].encode()).decode())
except Exception:
await call.answer(”[ERR] Ошибка”, show_alert=True)
return
...
kb = InlineKeyboardMarkup(inline_keyboard=[
[InlineKeyboardButton(text=”[ERR] Удалить”, callback_data=f”delchat*{base64.b64encode(str(chat.chat_id).encode()).decode()}”)],
...
await call.answer(f”[OK] Чат {chat.title} удалён”, show_alert=True)
else:
await call.answer(”[ERR] Чат не найден”, show_alert=True)
...
await call.answer(”[OK] Все чаты удалены”, show_alert=True)
await list_chats(call)
...
await message.answer(”[ERR] Требуется активная подписка.”)
await state.clear()
return
...
if message.text:
    ...
    await message.answer("[ERR] Неподдерживаемый тип.")
    return

await state.update_data(message_data=msg_data)
chats = await get_chats()
if not chats:
    await message.answer("[ERR] Нет добавленных чатов.")
    await state.clear()
    return

buttons = [[InlineKeyboardButton(text=f"?? {c.title}", callback_data=f"selchat_{base64.b64encode(str(c.chat_id).encode()).decode()}")] for c in chats]
buttons.append([InlineKeyboardButton(text="[OK] Готово", callback_data="done_selection")])
await message.answer("[LIST] <b>Выберите чаты для рассылки:</b>", reply_markup=InlineKeyboardMarkup(inline_keyboard=buttons))
...
chats = await get_chats()
buttons = [[InlineKeyboardButton(text=f”{’[OK]’ if c.chat_id in selected else ‘??’} {c.title}”, callback_data=f”selchat*{base64.b64encode(str(c.chat_id).encode()).decode()}”)] for c in chats]
buttons.append([InlineKeyboardButton(text=”[OK] Готово”, callback_data=“done_selection”)])
...
selected = data.get(‘selected_chats’, [])
if not selected:
await call.answer(”[ERR] Выберите хотя бы один чат”, show_alert=True)
return
kb = InlineKeyboardMarkup(inline_keyboard=[
[InlineKeyboardButton(text=”[>>] Начать”, callback_data=“confirm_broadcast”)],
[InlineKeyboardButton(text=”[ERR] Отмена”, callback_data=“cancel_broadcast”)]
])
await call.message.edit_text(f”[MAIL] Разослать в <b>{len(selected)}</b> чат(ов). Подтвердить?”, reply_markup=kb)
...
await state.clear()
await call.message.edit_text(”[ERR] Рассылка отменена.”)
await call.answer()
...
return
sessions = await get_all_sessions()
text = “[USER] <b>Управление сессиями</b>\n\n”
if sessions:
for s in sessions:
status = “[OK]” if (s.is_active and s.is_working) else “[ERR]”
...
```
kb = InlineKeyboardMarkup(inline_keyboard=[
    [InlineKeyboardButton(text="[MOB] Добавить аккаунт по номеру", callback_data="add_session_phone")],
    [InlineKeyboardButton(text="[FIND] Проверить все", callback_data="check_accounts")],
    [InlineKeyboardButton(text="[STAT] Статистика", callback_data="accounts_stats")],
    [InlineKeyboardButton(text="? Назад", callback_data="admin_panel")]
])
try:
    await call.message.delete()
...
await message.answer(”[ERR] Неверный формат. Пример: <code>+48500255953</code>”)
return
...
await message.answer("[MOB] Отправляю код в Telegram...")

asyncio.create_task(_create_session_task(message, state, phone, session_name))
```

async def _create_session_task(message: Message, state: FSMContext, phone: str, session_name: str):
client = None
try:
client = Client(
name=session_name,
...
```
    # Pyrogram 2.x: проверяем авторизацию через get_me()
    try:
        await client.get_me()
        await _finish_session(message, state, client, session_name)
        return
    except Exception:
        pass  # не авторизован - продолжаем

    sent_code = await client.send_code(phone)
    await state.update_data(phone_code_hash=sent_code.phone_code_hash, client=client)
    await message.answer("[OK] Код отправлен!\n\nВведите код из Telegram:")
    await state.set_state(SessionStates.waiting_code)

except Exception as e:
    err = str(e).lower()
    if "locked" in err:
        await message.answer("[ERR] Database locked. Подождите 10 сек и попробуйте снова.")
    else:
        await message.answer(f"[ERR] Ошибка подключения: {str(e)[:300]}")
    if client:
        try:
...
```
if not client:
    await message.answer("[ERR] Сессия утеряна. Начните заново.")
    await state.clear()
    return
...
try:
    ...
    await message.answer("[LOCK] Включена 2FA. Введите пароль:")
    await state.set_state(SessionStates.waiting_password)
except PhoneCodeInvalid:
    await message.answer("[ERR] Неверный код. Попробуйте ещё раз:")
except Exception as e:
    await message.answer(f"[ERR] Ошибка: {str(e)[:200]}")
    try:
        await client.disconnect()
    except Exception:
        pass
...
try:
    ...
    await message.answer("[ERR] Неверный пароль! Попробуйте ещё раз:")
except Exception as e:
    await message.answer(f"[ERR] Ошибка: {str(e)[:200]}")
    try:
        await client.disconnect()
...
ok = await add_session_safe(session_file, me.phone_number or “”, me.first_name, me.username)

```
    if ok:
        # Автоматически вступаем в все уже добавленные чаты
        await message.answer(
            f"[OK] Аккаунт добавлен!\n"
            f"[USER] {me.first_name or '-'}\n"
            f"[MOB] {me.phone_number or ''}\n\n"
        ...
        for chat in chats:
            ...
            icon = "[OK]" if ok_join else "[ERR]"
            join_results.append(f"{icon} {chat.title}")
            await asyncio.sleep(1.5)

        if join_results:
            await message.answer("[LIST] Результаты вступления:\n" + "\n".join(join_results))
    else:
        await message.answer("[!]? Сессия уже существует в базе.")
except Exception as e:
    await message.answer(f"[ERR] Ошибка при сохранении: {str(e)[:200]}")
finally:
    try:
        await client.disconnect()
    except Exception:
...
sessions = await get_all_sessions()
if not sessions:
await call.message.answer(”[ERR] Нет сессий”)
await call.answer()
return
await call.message.answer(f”[FIND] Проверяю {len(sessions)} аккаунтов…”)
...
for s in sessions:
    try:
        async with Client(s.session_name.replace('.session', ''), api_id=API_ID, api_hash=API_HASH, workdir=SESSIONS_DIR) as app:
            me = await app.get_me()
            live += 1
            lines.append(f"[OK] {s.phone} - {me.first_name or '?'}")
            await update_session_status(s.id, True)
    except Exception as e:
        dead += 1
        lines.append(f"[ERR] {s.phone} - {str(e)[:40]}")
        await update_session_status(s.id, False)

text = "[FIND] <b>Результаты</b>\n\n" + "\n".join(lines) + f"\n\n[OK] Живых: {live} | [ERR] Мёртвых: {dead}"
...
text = (
f”[STAT] <b>Статистика аккаунтов</b>\n”
f”????????????????\n”
f”[MOB] Всего: {stats[‘total’]}\n”
f”[OK] Активных: {stats[‘active’]}\n”
f”[ERR] Мёртвых: {stats[‘dead’]}\n”
f”????????????????\n”
f”[MAIL] Отправлено всего: {stats[‘total_messages’]}\n”
f”[STAT] Мощность сегодня: {stats[‘daily_capacity’]} сообщений\n”
f”????????????????”
)
kb = InlineKeyboardMarkup(inline_keyboard=[
...
await call.answer(”[OK] Счётчики сброшены!”, show_alert=True)
...
if not await admin_check(call):
return
await call.message.answer(”[DOOR] Введите ссылку или username чата (@channel или https://t.me/…):”)
...
sessions = await get_active_sessions()
if not sessions:
await message.answer(”[ERR] Нет активных сессий!”)
return
...
for s in sessions:
    ...
    try:
        ...
        results.append(f"{'[OK]' if ok else '[ERR]'} {s.phone}")
    except Exception as e:
        results.append(f"[ERR] {s.phone} - {str(e)[:50]}")
    finally:
        if client:
            try:
                await client.stop()
...
await message.answer("[LIST] Результаты:\n" + "\n".join(results))
await state.clear()
```
...
text = (
f”[STAT] <b>Статистика</b>\n”
f”????????????????\n”
f”[DIR] Чатов: {total_chats}\n”
f”[DATE] Последняя рассылка: {last_bc.strftime(’%d.%m.%Y %H:%M’) if last_bc else ‘нет’}\n”
f”[MAIL] Отправлено: {sent}\n”
f”[!]? Ошибок: {errors}\n”
f”????????????????\n”
f”[USERS] Пользователей: {user_stats[‘total’]} (активных: {user_stats[‘active’]})\n”
f”[$] Общий баланс: {user_stats[‘total_balance’]:.2f}?\n”
f”????????????????\n”
f”[MOB] Аккаунтов: {spam_stats[‘total’]} (живых: {spam_stats[‘active’]})\n”
f”[STAT] Мощность: {spam_stats[‘daily_capacity’]}/день”
)
...
return
kb = InlineKeyboardMarkup(inline_keyboard=[
[InlineKeyboardButton(text=”[TIME]? Задержка”, callback_data=“set_delay”),
InlineKeyboardButton(text=”[IMG]? Баннер”, callback_data=“change_banner”)],
[InlineKeyboardButton(text=”[$] Цены подписок”, callback_data=“change_subscription_prices”),
InlineKeyboardButton(text=”[CARD] Реквизиты”, callback_data=“change_payment_details”)],
[InlineKeyboardButton(text=”[USER] Админы”, callback_data=“manage_admins”),
InlineKeyboardButton(text=”? Очистить чаты”, callback_data=“clear_chats_confirm”)],
[InlineKeyboardButton(text=”? Назад”, callback_data=“admin_panel”)]
])
await call.message.edit_text(”[SET]? <b>Настройки</b>”, reply_markup=kb)
await call.answer()

@router.callback_query(F.data == “set_delay”)
async def set_delay_prompt(call: CallbackQuery, state: FSMContext):
await call.message.answer(”[TIME]? Введите задержку между сообщениями (сек, 1-30):”)
...
DEFAULT_DELAY = delay
await message.answer(f”[OK] Задержка: {delay} сек.”)
else:
await message.answer(”[ERR] Введите от 1 до 30.”)
except Exception:
await message.answer(”[ERR] Введите число.”)
await state.clear()
...
if not await admin_check(call):
return
await call.message.answer(”[IMG]? Отправьте изображение (banner.jpg) или видео (welcome.mp4):”)
...
await message.answer(f”[OK] Баннер обновлён.”)
...
await message.answer(f”[OK] Видео обновлено.”)
else:
await message.answer(”[ERR] Отправьте фото или .mp4”)
except Exception as e:
await message.answer(f”[ERR] Ошибка: {e}”)
await state.clear()
...
InlineKeyboardButton(text=”[ERR] Удалить”, callback_data=“remove_admin”)],
...
await call.message.answer(”[NOTE] Введите Telegram ID нового администратора:”)
...
await add_admin(uid)
await message.answer(f”[OK] Добавлен администратор <code>{uid}</code>.”)
except Exception:
await message.answer(”[ERR] Неверный ID.”)
await state.clear()

@router.callback_query(F.data == “remove_admin”)
async def remove_admin_prompt(call: CallbackQuery, state: FSMContext):
await call.message.answer(”[NOTE] Введите Telegram ID администратора для удаления:”)
...
await message.answer(f”[OK] Удалён администратор <code>{uid}</code>.”)
except Exception:
await message.answer(”[ERR] Неверный ID.”)
await state.clear()
...
await message.answer(f”[OK] Цена {data[‘price_plan’]} ? {price}?”)
except Exception:
await message.answer(”[ERR] Введите число.”)
await state.clear()
...
return
kb = InlineKeyboardMarkup(inline_keyboard=[
[InlineKeyboardButton(text=”[CARD] Карта”, callback_data=“edit_payment_card”)],
...
await call.message.edit_text(”[CARD] Выберите способ оплаты:”, reply_markup=kb)
await call.answer()
...
return
data = await state.get_data()
await message.answer(f”[OK] Реквизиты {data[‘payment_method’]} обновлены: {message.text.strip()}”)
await state.clear()
...
if not users:
await call.message.edit_text(”[USERS] Пользователей нет”)
await call.answer()
return
buttons = []
for u in users[:20]:
...
icon = “[LOCK]” if u.is_blocked else “[OK]”
...
await call.message.edit_text(”[USERS] <b>Пользователи</b>:”, reply_markup=InlineKeyboardMarkup(inline_keyboard=buttons))
await call.answer()
...
if not user:
await call.answer(”[ERR] Не найден”, show_alert=True)
return
sub = await get_user_subscription(uid)
sub_text = f”[OK] до {sub.end_date.strftime(’%d.%m.%Y’)}” if sub else “[ERR] Нет”
text = (
f”[USER] <b>ID:</b> <code>{uid}</code>\n”
f”<b>Имя:</b> {user.first_name or ‘-’}\n”
f”<b>Username:</b> @{user.username or ‘-’}\n”
f”<b>Статус:</b> {’[LOCK] Заблокирован’ if user.is_blocked else ‘[OK] Активен’}\n”
f”[$] <b>Баланс:</b> {(user.balance or 0.0):.2f}?\n”
f”[DATE] <b>Подписка:</b> {sub_text}”
)
kb = InlineKeyboardMarkup(inline_keyboard=[
...
[InlineKeyboardButton(text=”[DATE] +Подписка”, callback_data=f”addsub_{uid}”),
InlineKeyboardButton(text=”[ERR] Отменить подписку”, callback_data=f”cancelsub_{uid}”)],
[InlineKeyboardButton(
text=”[OPEN] Разблокировать” if user.is_blocked else “[LOCK] Заблокировать”,
...
if amount <= 0:
await message.answer(”[ERR] Сумма > 0”)
return
if action == “add”:
await update_balance(uid, amount, “admin_add”)
await message.answer(f”[OK] Пополнено {amount}?”)
else:
await update_balance(uid, -amount, “admin_sub”)
await message.answer(f”[OK] Списано {amount}?”)
except Exception:
await message.answer(”[ERR] Введите число”)
return
await state.clear()
...
await call.answer(”[OK] Разблокирован”, show_alert=True)
else:
await block_user(uid)
await call.answer(”[OK] Заблокирован”, show_alert=True)
...
return
kb = InlineKeyboardMarkup(inline_keyboard=[
[InlineKeyboardButton(text=“3 дня”, callback_data=f”subplan_{uid}*3days”),
InlineKeyboardButton(text=“Неделя”, callback_data=f”subplan*{uid}*week”)],
[InlineKeyboardButton(text=“Месяц”, callback_data=f”subplan*{uid}*month”),
InlineKeyboardButton(text=“Год”, callback_data=f”subplan*{uid}*year”)],
[InlineKeyboardButton(text=”? Отмена”, callback_data=f”userinfo*{uid}”)]
])
await call.message.edit_text(f”[DATE] Выберите план для {user.first_name}:”, reply_markup=kb)
await call.answer()
...
await call.answer(”[OK] Подписка выдана”, show_alert=True)
await call.message.answer(f”[OK] Подписка {plan} до {end.strftime(’%d.%m.%Y’)}”)
...
sub = await get_user_subscription(uid)
if not sub:
await call.answer(”[ERR] Нет подписки”, show_alert=True)
return
await update_subscription_status(sub.id, False)
await call.answer(”[OK] Подписка отменена”, show_alert=True)
...
text = “[CARD] <b>Способы оплаты</b>\n\n”
for m in methods:
text += f”{’[OK]’ if m.is_active else ‘[ERR]’} {m.name}\n”
if not methods:
text += “Нет настроенных способов\n”
kb = InlineKeyboardMarkup(inline_keyboard=[
[InlineKeyboardButton(text=”[+] Добавить”, callback_data=“add_payment_method”)],
[InlineKeyboardButton(text=”[LIST] Платежи”, callback_data=“view_payments”)],
[InlineKeyboardButton(text=”? Назад”, callback_data=“admin_panel”)]
])
await call.message.edit_text(text, reply_markup=kb)
await call.answer()
...
await call.message.answer(”[NOTE] Введите название способа оплаты:”)
...
await message.answer(”[OK] Способ оплаты добавлен.”)
except json.JSONDecodeError:
await message.answer(”[ERR] Неверный JSON.”)
return
await state.clear()
...
text = f”[DATE] <b>Активные подписки:</b> {len(subs)}\n\n”
for sub in subs[:20]:
...
await update_subscription_status(sub.id, False)
try:
await bot.send_message(sub.user_id, “[!]? Подписка истекла. Продлить: /start”)
except Exception:
pass
else:
await update_subscription_status(sub.id, False)
